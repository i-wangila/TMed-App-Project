...
class _ManageProviderAccountScreenState
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoRow(
            'Name',
            _profile.premises?.name ?? _user?.name ?? 'N/A',
          ),
          _buildInfoRow(
            'Email',
            _profile.premises?.email ?? _user?.email ?? 'N/A',
          ),
          _buildInfoRow(
            'Phone',
            _profile.premises?.phone ?? _user?.phone ?? 'N/A',
          ),
  ...
  Widget _buildBusinessInformation() {
    final premises = _profile.premises;
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: Colors.white,
        ...
        children: [
          Row(
            ...
            children: [
              ...
              TextButton.icon(
                onPressed: _editBusinessInformation,
                icon: const Icon(Icons.edit, size: 18),
                label: const Text('Edit'),
              ),
            ],
          ...
          const SizedBox(height: 16),
          if (premises != null) ...[
            _buildInfoRow('Business Name', premises.name),
            _buildInfoRow('Address', premises.address),
            _buildInfoRow('City', premises.city),
            _buildInfoRow('Business Phone', premises.phone),
            _buildInfoRow('Business Email', premises.email),
          ] else
            Text(
  ...
  void _editBusinessInformation() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) =>
            EditProviderBusinessInfoScreen(providerProfile: _profile),
      ),
    );
