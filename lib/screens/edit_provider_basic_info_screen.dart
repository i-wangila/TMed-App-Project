...
class _EditProviderBasicInfoScreenState
  ...
  void initState() {
    super.initState();
    // Initialize with premises data if available, otherwise use user data
    _nameController = TextEditingController(
      text: widget.providerProfile.premises?.name ?? widget.user?.name ?? '',
    );
    _emailController = TextEditingController(
      text: widget.providerProfile.premises?.email ?? widget.user?.email ?? '',
    );
    _phoneController = TextEditingController(
      text: widget.providerProfile.premises?.phone ?? widget.user?.phone ?? '',
    );
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      // Create or update premises with new information
      final premises = BusinessPremises(
        id: widget.providerProfile.premises?.id,
        name: _nameController.text.trim(),
        email: _emailController.text.trim(),
        phone: _phoneController.text.trim(),
        address: widget.providerProfile.premises?.address ?? '',
        city: widget.providerProfile.premises?.city ?? 'Nairobi',
        country: widget.providerProfile.premises?.country ?? 'Kenya',
        description: widget.providerProfile.premises?.description,
